				   ...
				   AVStream * input_stream)
{
	...
	output_codec_context->extradata_size =
	...
	case AVMEDIA_TYPE_VIDEO:
		...
		output_codec_context->has_b_frames =
		    input_codec_context->has_b_frames;
...
void output_transfer_command(const unsigned int first_segment,
			     const unsigned int last_segment, const int end,
			     const char *encoding_profile)
{
	char buffer[1024 * 10];
	memset(buffer, 0, sizeof(char) * 1024 * 10);

	sprintf(buffer, "%d, %d, %d, %s", first_segment, last_segment, end,
		encoding_profile);
